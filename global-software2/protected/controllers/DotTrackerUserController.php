...
class DotTrackerUserController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete', 'changePageSize'),
				'users'=>array('@'),
			),
	...
	public function actionChangePageSize() {
		$id = Yii::app()->request->getParam('id', 0);
        $pageSize = Yii::app()->request->getParam('pageSize', 0);
        $success = false;
        $msg = '';
        if(empty($id) || empty($pageSize)) {
            die;
        }
        $model = $this->loadModel($id);
        ...
        if($model->save(false)) {
        	$success = true;
            $msg = 'Page Size updated successfully';
        } else {
        	$success = false;
            $msg = 'Page Size could not be updated';
