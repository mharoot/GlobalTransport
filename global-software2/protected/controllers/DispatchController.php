...
class DispatchController extends Controller {
    ...
    public function actionDispatch($id, $status) {
        ...
        if(isset($_POST['GlobalTrackerDispatch'])) {
            $model->attributes = $_POST['GlobalTrackerDispatch'];
                ...
                foreach ($model as $name => $value) {
                    if($name == 'vehicle_info') {
                        ...
                        if($name == 'save_as_default') {
                            $old = GlobalTrackerDispatch::$arrSaveAsDefault[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$arrSaveAsDefault[$value];
                        } else if($name == 'status') {
                            $old = GlobalTrackerDispatch::$arrStatus[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$arrStatus[$value];
                        }
	...
	public function actionCreate() {
		$model = new GlobalTrackerDispatch;
        $sendSms = false;
        $sendEmail = false;
                    ...
                    if($name == 'vehicle_info') {
                        ...
                        for($i=1; $i<=5; $i++) {
                            ...
                            if($obj->{'Vehicle_type'.$i} != '') {
                                $this->insertHistoryO($model->id,'Vehicle_type'.$i,'none',GlobalTrackerDispatch::$enumbVehicleType[$obj->{'Vehicle_type'.$i}],$curTime,$model->created_by);
                            }
                        ...
                        $new = $value;
                        if($name == 's_vrun') {
                            $new = GlobalTrackerDispatch::$arrVRun[$model->$name];
                        } else if($name == 's_via') {
                            $new = GlobalTrackerDispatch::$enumShipVia[$model->$name];
                        } else if($name == 'referred_by') {
                            $new = GlobalTrackerDispatch::$enumReferBy[$model->$name];
                        } else if($name == 'status') {
                            $new = GlobalTrackerDispatch::$arrStatus[$model->$name];
                        } else if($name == 'save_as_default') {
                            $new = GlobalTrackerDispatch::$arrSaveAsDefault[$model->$name];
                        }
                    ...
                    if($name == 'vehicle_info') {
                        ...
                        for($i=1; $i<=5; $i++) {
                            ...
                            if($oldObj->{'Vehicle_type'.$i} != $newObj->{'Vehicle_type'.$i}) {
                                $this->insertHistoryO($model->id,'Vehicle_type'.$i,GlobalTrackerDispatch::$enumbVehicleType[$oldObj->{'Vehicle_type'.$i}],GlobalTrackerDispatch::$enumbVehicleType[$newObj->{'Vehicle_type'.$i}],$curTime,$model->created_by);
                            }
                        ...
                        if($name == 's_vrun') {
                            $old = GlobalTrackerDispatch::$arrVRun[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$arrVRun[$value];
                        } else if($name == 's_via') {
                            $old = GlobalTrackerDispatch::$enumShipVia[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$enumShipVia[$value];
                        } else if($name == 'referred_by') {
                            $old = GlobalTrackerDispatch::$enumReferBy[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$enumReferBy[$value];
                        } else if($name == 'status') {
                            $old = GlobalTrackerDispatch::$arrStatus[$oldValues->$name];
                            $new = GlobalTrackerDispatch::$arrStatus[$value];
                        } else if($name == 'save_as_default') {
	...
	public function actionAdmin() {
		$model=new GlobalTrackerDispatch('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['GlobalTrackerDispatch']))
			$model->attributes=$_GET['GlobalTrackerDispatch'];
	 ...
	 * @return GlobalTrackerDispatch the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id) {
		$model = GlobalTrackerDispatch::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param GlobalTrackerDispatch $model the model to be validated
	 */
    ...
    public function sendSaveEmailToUser($model) {
        ...
        $txt = "Hi Admin,<br><br>Signature submitted for Order Form.<a href='http://globaltransportationsoftware.com/index.php?r=dispatch/view&id=".$model->id."'>Click here</a> to view <br><br> Warm Regards<br>Support Team<br> Global Auto Transportation";
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From: info@globalautotransportation.com" . "\r\n";
    ...
    public function sendEmailToUser($CreditCardAuth) {
        ...
        $arr = GlobalTrackerDispatch::model()->attributeLabels();
        ...
        foreach ($arr as $index=>$val) {
            ...
            if($index == 's_via') {
                $body = str_replace('[s_via]', GlobalTrackerDispatch::$enumShipVia[$allAttr['s_via']], $body);
            }
        ...
        $body = str_replace('[auth_link]','<a href="http://globaltransportationsoftware.com/index.php?r=dispatch/authorizeSignature&id='.$allAttr['id'].'">Click here to Authorize Order</a>',$body);
    ...
    public function sendSmsToUser($CreditCardAuth) {
        ...
        $body = str_replace('[auth_link]', 'http://globaltransportationsoftware.com/index.php?r=dispatch/authorizeSignature&id='.$allAttr['id'], $body);
    ...
    public function actionSaveScr() {
        ...
        $CreditCardAuth=GlobalTrackerDispatch::model()->findByPk($id);
